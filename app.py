...
class AppConfig:
    ...
    SQLALCHEMY_DATABASE_URI = os.getenv(
        'DATABASE_URI', 'mysql+pymysql://root:admin@localhost/tcc_db'
    )
...
class User(UserMixin, db.Model):
    __tablename__ = 'usuarios'  # ← MUDE de 'users' para 'usuarios'
    ...
    curso_id = db.Column('id_curso', db.Integer, db.ForeignKey('cursos.id'), nullable=True)  # ← ADICIONE 'id_curso'
...
class TCC(db.Model):
    ...
    user_id = db.Column('id_autor', db.Integer, db.ForeignKey('usuarios.id'), nullable=False)  # ← ADICIONE 'id_autor'
    user = db.relationship('User', backref='tccs', lazy=True)
    orientador_id = db.Column('id_orientador', db.Integer, db.ForeignKey('orientador.id'), nullable=True)  # ← ADICIONE 'id_orientador'
...
class Download(db.Model):
    ...
    tcc_id = db.Column(db.Integer, db.ForeignKey('tccs.id'), nullable=False)
...
class RegisterForm(FlaskForm):
    ...
    curso = SelectField('Curso', coerce=int, validators=[DataRequired()])
